  ...
  var demo = function() {
        ...
        },
        {
        ...
        },
         {
          ...
          template: function(row) {
            ...
            if(row.status == 1){
              ico = '<i class="la la-unlock-alt"></i>';
              title = 'Activate';
            } else{
              ico = '<i class="la la-unlock"></i>';
              title = 'Deactivate';
            }

            return '\
            <a href="'+adminpath+'#/cms-pages/edit/'+row.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit details">\
    ...
    $('#delselall').on('click',function(){
      var url = apipath+'deletecmspage';
        ...
        if(leadids.length > 0){
            doIt = confirm('Are you sure you want to delete selected pages?');
      if (doIt) {
